...
public class FbBot extends Bot {
    ...
    public void onGetStarted(Event event) {
        ...
        reply(event, "Como vai, " + event.getRecipient().getFirstName());
     ...
     * calls {@link IBot#startConversation(Event, String)} within it. You can chain methods which will be invoked
     ...
     * call {@link IBot#nextConversation(Event)} to jump to the next question in the conversation.
     *
     * @param event
     */
     ...
     * call {@link IBot#stopConversation(Event)} to end the conversation.
     *
     * @param event
     */
